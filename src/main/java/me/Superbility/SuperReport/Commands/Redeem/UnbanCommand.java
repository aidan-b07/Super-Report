package me.Superbility.SuperReport.Commands.Redeem;
...
public class UnbanCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if(cmd.getName().equalsIgnoreCase("unban")) {
            if(sender.hasPermission("superreport.unban")) {
                if (args.length > 0) {
                    try {
                        String stringPlayer = args[0];
                        OfflinePlayer player = Bukkit.getOfflinePlayer(stringPlayer);
                        PreparedStatement getConnectionValues = values.getConnection().prepareStatement("SELECT `REASON`,`PUNISHER_UUID` FROM " + values.getTablePunishmentBan() + " WHERE UUID = '" + player.getUniqueId() + "'");
                        ...
                        if (valuesSet.next()) {
                            ...
                            sender.sendMessage(ChatColor.translateAlternateColorCodes('&', mc.getPunishmentsCfg().getString("UnbanCommand.Successful")));
                            return true;
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
